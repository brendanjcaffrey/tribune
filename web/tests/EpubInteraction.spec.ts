...
test("marks read the first time the end is reached", () => {
  const ref = tracking();
  expect(EpubInteraction.shouldMarkRead(ref, scroll(50, false))).toBe(false);
  expect(EpubInteraction.shouldMarkRead(ref, scroll(100, true))).toBe(true);
});

test("does not mark read again while sitting on the last page", () => {
  const ref = tracking();
  expect(EpubInteraction.shouldMarkRead(ref, scroll(100, true))).toBe(true);
  expect(EpubInteraction.shouldMarkRead(ref, scroll(100, true))).toBe(false);
});

test("does not mark read when a footnote link jumps to the end", () => {
  const ref = tracking();
  EpubInteraction.shouldMarkRead(ref, scroll(20, false));
...
test("marks read after a footnote jump once the end is reached by reading", () => {
  ...
  expect(EpubInteraction.shouldMarkRead(ref, scroll(40, false))).toBe(false);
